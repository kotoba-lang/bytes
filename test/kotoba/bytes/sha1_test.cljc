(ns kotoba.bytes.sha1-test
  "FIPS 180-4 SHA-1 vectors + RFC 2202 HMAC-SHA1 vectors against the pure,
   portable `kotoba.bytes.sha1` implementation — the same code the
   ClojureScript branch of `kotoba.turn.credential` calls directly. See
   `credential_test.clj` (in kotoba-lang/turn) for the cross-check against the
   JVM `javax.crypto` branch."
  (:require [clojure.test :refer [deftest is testing]]
            [kotoba.bytes :as b]
            [kotoba.bytes.sha1 :as sha1]))

(defn- hex
  "Lowercase 2-digit-per-byte hex string, matching `(format \"%02x\" b)` output
   byte-for-byte. Written portably (no `format`, which is JVM-only and not
   available under :cljs) since this test now runs as `.cljc`."
  [bs]
  (apply str (map (fn [byte]
                     (let [b (bit-and byte 0xff)
                           s #?(:clj (Integer/toHexString b) :cljs (.toString b 16))]
                       (if (< b 0x10) (str "0" s) s)))
                   bs)))
